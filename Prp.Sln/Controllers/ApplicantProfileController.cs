    ...
    public class ApplicantProfileController : BaseController
    {
            ...
            try
            {
                ...
                else
                {
                   ...
                   , ProjConstant.Constant.pakistan);
                    ...
                    model.listInstitute = new InstitueDAL().GetAll(ProjConstant.Constant.InstituteType.govt);
                    model.listDiscipline = new CommonDAL().DisciplineGetAll();
                    model.listSpeciality = new CommonDAL().SpecialityGetAll();
                    return View(model);
                }
            }
            catch (Exception)
        ...
        public ProofReadingModel GenerateModelProofReading()
        {
            ...
            try
            {
                ...
                model.listAppicantLevel3 = new ApplicantDAL().GetApplicantLevel3Detail(0, 0, loggedInUser.applicantId);// GetApplicantCertificateListDetail(0, 0, loggedInUser.applicantId);
        ...
        public JsonResult ApplicantEducationAddUpdate(ApplicantEducationParam objEducation)
        {
            ...
            if (msg.status == true)
            {
                ...
                if (objEducation.listLevel3 != null && objEducation.listLevel3.Count > 0)
                {
                    foreach (var item in objEducation.listLevel3)
                    {
                        item.inductionId = ProjConstant.inductionId;
                        item.phaseId = ProjConstant.phaseId;
                        if(item.disciplineId == 0)
                        {
                            item.disciplineId= new CommonDAL().GetSpecialityIdBydisciplineId(item.specialityId);                      
                            }
                    }
                    msg = new ApplicantDAL().ApplicantLevel3AddUpdate(objEducation.listLevel3);
                }
            }
        ...
        [HttpPost]
        public JsonResult AddToInstitute(string instituteName)
        {
            Message msg = new Message();
            string query = "INSERT INTO tblInstitute (name, code, instituteTypeId, provinceId, districtId, sortOrder, isActive,dated,adminId) "
                +" VALUES('"+instituteName+"' "
                +" , '0000', 3, 196, 204, 13, 1, GETDATE(), 1) "
            ...
            try
            {
                ...
                con.Open();
                cmd.Connection = con;
                msg.id = Convert.ToInt32(cmd.ExecuteScalar());
                msg.status = true;
            }
            catch (Exception ex)
            {
        ...
        [HttpGet]
        public JsonResult GetApplicantLevel3Details(int applicantId)
        {
            var list = new ApplicantDAL().GetApplicantLevel3Details(applicantId);
            return Json(list, JsonRequestBehavior.AllowGet);
        }
        ...
        public JsonResult ApplicantExperienceAddUpdate(ApplicantExperienceParam objExperience)
        {
            ...
            obj.specialityId = objExperience.specialityId;
            obj.superSpecialtyId = Convert.ToInt32(objExperience.superSpecialityId);
        ...
        public JsonResult ApplicantResearchPaperAddUpdate(ApplicantResearchPaperParam objExperience)
        {
            ...
            obj.researchType = objExperience.researchType;
        ...
        {
            List<ApplicantResearchPaper> list = new ApplicantDAL().GetApplicantResearchPaperList(ProjConstant.inductionId, ProjConstant.phaseId, applicantId);
            return Json(list, JsonRequestBehavior.AllowGet);
        }
